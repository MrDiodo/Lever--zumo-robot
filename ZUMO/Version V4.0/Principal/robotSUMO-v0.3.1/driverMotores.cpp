...
void driverMotores::fwdTask(int mil, float args){
  motorTask = 0; //fwdFull
  taskArgs = args; //Se pasa la potencia del fwdFull
  setInterval(mil); //En cuanto tiempo se debe llamar a fwdFull
  runned();
  enabled = true;
...
void driverMotores::fwdTurn(float deg, int mil) //Giro hacia adelante (toma un valor desde -1 a 1 y el tiempo de giro)
{
    M1_Potencia = min(255, (255 * (deg + 1)));
    M2_Potencia = min(255, (255 * (1 - deg)));
    fwdTask(mil, 1);
}
void driverMotores::bwdTurn(float deg, int mil) //Giro hacia atras (toma un valor desde -1 a 1 y el tiempo de giro)
{
