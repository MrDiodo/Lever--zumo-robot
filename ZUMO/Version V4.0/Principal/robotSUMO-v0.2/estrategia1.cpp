...
void estrategia1::execLinea(int sensor)
{
    ...
    {
        (*_driver_pointer).fwdTurn(0.8); //Atras izq.
        (*_driver_pointer).updateMotors();
        delay(300);
    }
    else if (sensor == _entLin[1]) // Pin 12
    {
        ...
        delay(600);
        (*_driver_pointer).onSpotTurn(-90); //Adelante izq.
    }
    else if (sensor == _entLin[2]) // Pin 11
    {
        (*_driver_pointer).bwdFull();
        delay(600);
        (*_driver_pointer).onSpotTurn(90); //Adelante der.
    }
    else if (sensor == _entLin[3]) // Pin 10
    {
        (*_driver_pointer).fwdTurn(-1 * 0.8); //Atras der.
        delay(300);
    }
    else
    {
        (*_driver_pointer).fwdFull();
    }
}
...
void estrategia1::execIR(int sensor) //Los sensores estan ordenados de mayor a menor importancia
{
    ...
    {
        (*_driver_pointer).fwdFull(); //N
        //(*_driver_pointer).fwdTurn(0.8); //NO
    }
    else if (sensor == _entIR[3])
    {
        (*_driver_pointer).fwdFull(); //N
        //(*_driver_pointer).fwdTurn(-0.8); //NE
    }
